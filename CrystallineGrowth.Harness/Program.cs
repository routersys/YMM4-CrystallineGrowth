...
static int Structure(HarnessImage image)
{
    ...
    var variants = new (string Name, Func<bool, CrystallineGrowthPipeline.Parameters> Make)[]
    {
        ("cached", _ => parameters),
        ("branching", flip => parameters with { Branching = flip ? 0.70f : 0.60f }),
        ("facet", flip => parameters with { Facet = flip ? 0.50f : 0.40f }),
        ("reach", flip => parameters with { ReachPixels = flip ? 176f : 160f }),
        ("noise", flip => parameters with { Noise = flip ? 0.35f : 0.25f }),
        ("seed", flip => parameters with { Seed = flip ? 8 : 7 }),
        ("all", flip => parameters with
        {
            Branching = flip ? 0.70f : 0.60f,
            Facet = flip ? 0.50f : 0.40f,
            ReachPixels = flip ? 176f : 160f,
            Noise = flip ? 0.35f : 0.25f,
            Seed = flip ? 8 : 7,
        }),
    };
