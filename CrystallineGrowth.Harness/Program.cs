...
foreach (var freeze in new[] { 0.25f, 0.5f, 0.75f, 1f })
{
    var parameters = new CrystallineGrowthPipeline.Parameters(CrystallineGrowthQuality.High, freeze, 0.6f, 0.4f, 160f, 0.25f, 0.7f, 0.4f, 0.5f, 0.8f, 0.89f, 1f, 7);
    pipeline.Process(source, destination, width, height, in parameters);
    Console.WriteLine($"freeze={freeze:F2} litPixels={CountLit(destination)}");
    WriteBmp(Path.Combine(outputDirectory, $"freeze{(int)(freeze * 100):D3}.bmp"), Composite(source, destination), width, height);
}

foreach (var branching in new[] { 0f, 0.5f, 1f })
{
    var parameters = new CrystallineGrowthPipeline.Parameters(CrystallineGrowthQuality.High, 1f, branching, 0.4f, 160f, 0.25f, 0.7f, 0.4f, 0.5f, 0.8f, 0.89f, 1f, 7);
    pipeline.Process(source, destination, width, height, in parameters);
...
foreach (var facet in new[] { 0f, 0.5f, 1f })
{
    var parameters = new CrystallineGrowthPipeline.Parameters(CrystallineGrowthQuality.High, 1f, 0.6f, facet, 160f, 0.25f, 0.7f, 0.4f, 0.5f, 0.8f, 0.89f, 1f, 7);
    pipeline.Process(source, destination, width, height, in parameters);
    Console.WriteLine($"facet={facet:F2} litPixels={CountLit(destination)}");
    WriteBmp(Path.Combine(outputDirectory, $"facet{(int)(facet * 100):D3}.bmp"), Composite(source, destination), width, height);
}
...
static int[] Composite(int[] source, int[] ice)
{
