...
public sealed class CrystallineGrowthEffectProcessorTests
{
    ...
    [InlineData(100, 50)]
    [InlineData(-37, 21)]
    public void TheFrostTravelsWithTheImage(int dx, int dy)
    {
        using var devices = new GraphicsDevices();
        using var context = devices.CreateContext();
        RequireInterop(context);
        ...
        using var inPlaceProcessor = new CrystallineGrowthEffect().CreateVideoEffect(context);
        inPlaceProcessor.SetInput(source.Bitmap);
        var inPlace = RenderFrame(context, inPlaceProcessor, 0);
        using var travelledProcessor = new CrystallineGrowthEffect().CreateVideoEffect(context);
