...
[Collection("Direct2D")]
public sealed class CrystallineGrowthInteropTests
{
    static readonly Bgra Gray = Bgra.Opaque(192, 192, 192);

    static CrystallineGrowthPipeline.Parameters Parameters()
        => new(CrystallineGrowthQuality.Balanced, 1f, 0.6f, 0.4f, 40f, 0.25f, 0.7f, 0.4f, 0.5f, 0.8f, 0.89f, 1f, 0);
    ...
    sealed class Interop : IDisposable
    {
        ...
        readonly CrystallineGrowthInteropProvider provider;
        readonly ComputeInteropDomain domain;

        public CrystallineGrowthResourceSet Resources { get; }

        public CrystallineGrowthPipeline Pipeline { get; }

        Interop(ComputeExternalQueueScheduler scheduler, CrystallineGrowthInteropProvider provider, GraphicsDevice device)
        {
            this.scheduler = scheduler;
            this.provider = provider;
            domain = device.RegisterExternalDomain(provider);
            Resources = CrystallineGrowthResourceSet.Create(device, domain);
            Pipeline = CrystallineGrowthPipeline.TryCreate(device)!;
        }

        public static Interop Create(IGraphicsDevicesAndContext devices)
        {
            var scheduler = ComputeExternalQueueScheduler.Create();
            var provider = CrystallineGrowthInteropProvider.TryCreate(devices, scheduler, out var device);
    ...
    static int LitPixels(Rendering rendering, CrystallineGrowthPipeline.PixelRect visible)
    {
        var lit = 0;
    ...
    [Fact]
    public void FrostIsDrawnAfterTheOutputGrowsToFullHd()
    {
        ...
        CrystallineGrowthPipeline.PixelRect RenderInto(Func<CrystallineGrowthPipeline.PixelRect, (int Width, int Height)> outputSize)
        {
            ...
            interop.Pipeline.RenderVisible(interop.Resources.GetSourceComputeBinding(), interop.Resources.GetOutputComputeBinding(), 96, 96, 0, 0, 96, 96, visible, in parameters);
            return visible;
        }
