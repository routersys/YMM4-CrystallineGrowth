...
public sealed class CrystallineGrowthPipelineTests
{
    ...
    static CrystallineGrowthPipeline.Parameters Parameters(
    ...
    static int[] Render(CrystallineGrowthPipeline pipeline, int[] source, int width, int height, CrystallineGrowthPipeline.Parameters parameters)
    {
    ...
    [Fact]
    public void ATransparentSourceProducesNoFrost()
    {
    ...
    [Fact]
    public void AFullFreezeProducesFrost()
    {
        using var pipeline = CreatePipeline();
    ...
    [Fact]
    public void TheSameSettingsAlwaysProduceTheSameFrost()
    {
        using var pipeline = CreatePipeline();
        var source = Square(128, 128, 48, 48, 32, 32);

        var first = Render(pipeline, source, 128, 128, Parameters(branching: 0.8f, noise: 0.5f, seed: 42));
        var second = Render(pipeline, source, 128, 128, Parameters(branching: 0.8f, noise: 0.5f, seed: 42));

        Assert.Equal(first, second);
    }

    [Fact]
    public void ADifferentSeedGrowsTheCrystalsDifferently()
    {
        using var pipeline = CreatePipeline();
        var source = Square(128, 128, 48, 48, 32, 32);

        var first = Render(pipeline, source, 128, 128, Parameters(noise: 0.5f, seed: 1));
        var second = Render(pipeline, source, 128, 128, Parameters(noise: 0.5f, seed: 2));

        Assert.NotEqual(first, second);
    }

    [Fact]
    public void TheFrostStaysPremultiplied()
    {
        using var pipeline = CreatePipeline();

        var rendering = Render(pipeline, Square(128, 128, 48, 48, 32, 32), 128, 128, Parameters(frost: 1f, refraction: 1f, specular: 1f));
    ...
    [Fact]
    public void AFurtherFreezeCoversMorePixels()
    {
        using var pipeline = CreatePipeline();
        var source = Square(128, 128, 48, 48, 32, 32);

        var partial = Render(pipeline, source, 128, 128, Parameters(freeze: 0.2f));
        var full = Render(pipeline, source, 128, 128, Parameters(freeze: 1f));

        Assert.True(LitPixels(full) > LitPixels(partial));
    }

    [Fact]
    public void TheReachLimitsHowFarTheFrostGrows()
    {
        using var pipeline = CreatePipeline();
        var source = Square(192, 192, 80, 80, 32, 32);

        var shortReach = LowestLitRow(Render(pipeline, source, 192, 192, Parameters(reachPixels: 12f)), 192);
        var longReach = LowestLitRow(Render(pipeline, source, 192, 192, Parameters(reachPixels: 48f)), 192);
    ...
    [Fact]
    public void SharedTexturesProduceTheSameFrostAsPackedBuffers()
    {
        using var pipeline = CreatePipeline();
        var source = Square(96, 96, 32, 32, 32, 32);
        var parameters = Parameters(branching: 0.7f, seed: 11);
    ...
    [InlineData(0.3f)]
    [InlineData(1f)]
    public void TheVisibleBoundsHoldEveryFrostedPixelAndRenderTheSameFrost(float freeze)
    {
        using var pipeline = CreatePipeline();
        var source = Square(192, 192, 80, 80, 32, 24);
        var parameters = Parameters(freeze: freeze, reachPixels: 32f, seed: 5);
    ...
    [Fact]
    public void ALesserFreezeNarrowsTheVisibleBounds()
    {
        using var pipeline = CreatePipeline();
        var device = GraphicsDevice.GetDefault();
        using var sourceTexture = device.AllocateReadWriteTexture2D<Bgra32, Float4>(192, 192);
    ...
    [InlineData(false, 1f)]
    [InlineData(true, 0f)]
    public void NothingIsVisibleWithoutFrostOrFreeze(bool shaped, float freeze)
    {
        using var pipeline = CreatePipeline();
        var device = GraphicsDevice.GetDefault();
