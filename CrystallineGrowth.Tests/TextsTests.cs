...
public sealed class TextsTests
{
    ...
    {
        var neutral = Texts.ResourceManager.GetString(nameof(Texts.CrystallineGrowth), CultureInfo.InvariantCulture);

        Assert.Equal("氷晶成長", neutral);
        Assert.Equal(neutral, Texts.ResourceManager.GetString(nameof(Texts.CrystallineGrowth), CultureInfo.GetCultureInfo("ja-JP")));
    }
