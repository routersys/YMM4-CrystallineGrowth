...
internal sealed class CrystallineGrowthEffectProcessor : VideoEffectProcessorBase
{
    ...
    private CrystallineGrowthGpuInterop? _interop;
    ...
    {
        var interop = CrystallineGrowthGpuInterop.TryCreate(devices);
        if (interop is null)
            return null;
        var pipeline = CrystallineGrowthPipeline.TryCreate(interop.Device);
        if (pipeline is null)
        {
            interop.Dispose();
            return null;
