...
internal sealed class CrystallineGrowthEffectProcessor : VideoEffectProcessorBase
{
    private readonly IGraphicsDevicesAndContext _devices;
    private readonly CrystallineGrowthEffect _item;
    private ComputeExternalQueueScheduler? _scheduler;
    private CrystallineGrowthInteropProvider? _interopProvider;
    private ComputeInteropDomain? _interopDomain;
    private CrystallineGrowthResourceSet? _resourceSet;
    private ExternalTextureLease<ExternalDirect3D11TextureView>? _outputLease;
    private CrystallineGrowthPipeline? _pipeline;
    private CrystallineGrowthCustomEffect? _effect;
    ...
    public CrystallineGrowthEffectProcessor(IGraphicsDevicesAndContext devices, CrystallineGrowthEffect item)
        : base(devices)
    {
        _devices = devices;
        _item = item;
    ...
    private DrawDescription UpdateCore(EffectDescription effectDescription)
    {
        ...
        var parameters = new Parameters(
            ...
            (float)(_item.Freeze.GetValue(frame, length, fps) / 100.0),
            _item.Quality,
            (float)(_item.Branching.GetValue(frame, length, fps) / 100.0),
            (float)(_item.Facet.GetValue(frame, length, fps) / 100.0),
            (float)(_item.Reach.GetValue(frame, length, fps) / 100.0),
            (float)(_item.Noise.GetValue(frame, length, fps) / 100.0),
            (float)(_item.Frost.GetValue(frame, length, fps) / 100.0),
            (float)(_item.Refraction.GetValue(frame, length, fps) / 100.0),
            (float)(_item.Specular.GetValue(frame, length, fps) / 100.0),
            _item.IceColor,
            _item.Seed);
        ...
        if (parameters.Amount <= 0f || parameters.Freeze <= 0f)
        {
            _effect.Amount = 0f;
        ...
        var marginLimit = (CrystallineGrowthSettings.MaximumCanvasSize - longSide) / 2d;
        if (marginLimit < 16d)
        {
            _effect.Amount = 0f;
            _isFirst = true;
        ...
        var pipelineParameters = new CrystallineGrowthPipeline.Parameters(
            parameters.Quality,
            Math.Clamp(parameters.Freeze, 0f, 1f),
            Math.Clamp(parameters.Branching, 0f, 1f),
            Math.Clamp(parameters.Facet, 0f, 1f),
            (float)reachPixels,
            Math.Clamp(parameters.Noise, 0f, 1f),
            Math.Clamp(parameters.Frost, 0f, 1f),
            Math.Clamp(parameters.Refraction, 0f, 1f),
    ...
    protected override ID2D1Image? CreateEffect(IGraphicsDevicesAndContext devices)
    {
        ...
        var interopProvider = CrystallineGrowthInteropProvider.TryCreate(devices, scheduler, out var interopDevice);
        ...
        CrystallineGrowthCustomEffect? effect = null;
        ...
        try
        {
            effect = new CrystallineGrowthCustomEffect(devices);
            if (!effect.IsEnabled)
            {
                effect.Dispose();
                ReleaseInterop();
