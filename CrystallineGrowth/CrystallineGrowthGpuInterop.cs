...
internal sealed class CrystallineGrowthInteropProvider : ComputeExternalDirect3D11Provider
{
    private readonly ID2D1DeviceContext6 _renderContext;

    private CrystallineGrowthInteropProvider(
    ...
    public static CrystallineGrowthInteropProvider? TryCreate(
        ...
        out GraphicsDevice? graphicsDevice)
    {
        ...
        try
        {
            ...
            var provider = new CrystallineGrowthInteropProvider(device, context, renderContext, scheduler);
            renderContext = null;
            return provider;
        }
        catch
...
internal sealed partial class CrystallineGrowthResourceSet
{
